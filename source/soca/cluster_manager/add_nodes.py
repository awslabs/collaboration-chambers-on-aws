...
publicVpc = aligo_configuration['PublicVpc'] == 'true'
if publicVpc:
    servicequotas = boto3.client("service-quotas")
else:
    # No VPC endpoint for servicequotas
    servicequotas = None
...
def verify_vcpus_limit(instance_type, desired_capacity, quota_info):
    if not servicequotas:
        return {'message': True, 'quota_info': {}}
...
def main(**kwargs):
    try:
        ...
        parameters_list = {
            ...
            'ErrorSnsTopicArn': {
                'Key': 'ErrorSnsTopicArn',
                'Default': aligo_configuration['ErrorSnsTopicArn']
            },
            'ESDomainEndpoint': {
                'Key': None,
