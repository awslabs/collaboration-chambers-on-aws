...
function on_exit {
    ...
    if [[ $rc -ne 0 ]]; then
        ...
        aws sns publish --topic-arn ''' + params['ErrorSnsTopicArn'] + ''' --subject "''' + params['ClusterId'] + ''' job ''' + str(params['JobId']) + ''' ComputeNode UserData failed" --message "See /var/log/cloud-init.log or grep cloud-init /var/log/messages | less for more info."
    fi
}
trap on_exit EXIT
