...
def create():
    ...
    if parameters['subnet_id'] is not False:
        ...
        if soca_configuration["PrivateSubnet3"]:
            soca_private_subnets.append(soca_configuration["PrivateSubnet3"])
...
function on_exit {
    ...
    if [[ $rc -ne 0 ]]; then
        ...
        aws sns publish --topic-arn ${!ErrorSnsTopicArn} --subject "${!ClusterId} ''' + str(session["user"]) + ''' Desktop UserData failed" --message "See /var/log/cloud-init.log or grep cloud-init /var/log/messages | less for more info."
    fi
}
trap on_exit EXIT
...
export publicVpc=''' + soca_configuration['PublicVpc'] + '''
...
echo export "SOCA_PUBLIC_VPC="''' + str(soca_configuration['PublicVpc']) +'''"" >> /etc/environment
